      ...
      <form action={action} className="space-y-0">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <button
              type="button"
              onClick={() => router.back()}
              className="mb-3 flex items-center gap-1.5 text-[0.8125rem] font-medium text-muted calm-transition hover:text-text"
            >
            ...
            <h1 className="text-[1.5rem] font-bold tracking-tight text-text">Review</h1>
            <p className="mt-1 text-[0.875rem] text-muted">Tap any row to revise a rating before submitting.</p>
          </div>
          {/* Completion summary */}
        ...
        <div className="overflow-hidden rounded-2xl border border-white/60 bg-white/70 backdrop-blur-sm">
          {orderedSignals.map((signal, index) => {
            ...
            const isUnanswered = !state?.valueKey && !state?.notObserved;
        ...
        <div className="mt-5 space-y-2">
          <label className="text-[0.6875rem] font-semibold uppercase tracking-[0.07em] text-muted">
            Context note <span className="font-normal normal-case tracking-normal">· optional</span>
          </label>
          <textarea
            name="contextNote"
